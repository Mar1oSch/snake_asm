...
; Finally the unit is also represented by a character on the board. I wanted to differentiate between the head and the rest. That's why the snake needs to tell the unit, if it is the head of the snake or it isn't. The unit uses that information to choose, which char is used to represent it.
unit_new:
.set_up:
...
.get_character:
    ...
    mov rcx, [rbp + 24]
    call _handle_char
    ; Save the returned char into the last local variable space.
    mov [rbp - 24], al
...
;;;;;; PRIVATE FUNCTIONS ;;;;;;
_handle_char:
    ; Expect 1 if head, 0 if not in RCX.
    test rcx, rcx
    jz .normal
